...
int main() {
    ...
    if (!webcpp_init(&config)) {
        fprintf(stderr, "Error: Failed to initialize WebCpp for Contacts app.\n");
        return 1;
    }

    webcpp_context_t* ctx = webcpp_create_context();
    if (!ctx) {
        fprintf(stderr, "Error: Failed to create WebCpp context for Contacts app.\n");
        webcpp_shutdown();
        return 1;
    }

    // Load the Contacts.html file for the application's UI
    // This uses the function we previously added to webcpp.h and webcpp.c
    if (!webcpp_load_html_file(ctx, "Contacts.html")) {
        fprintf(stderr, "Error: Contacts app failed to load Contacts.html\n");
        webcpp_destroy_context(ctx);
        webcpp_shutdown();
        return 1;
    }
    printf("Contacts app: Contacts.html loaded successfully.\n");
